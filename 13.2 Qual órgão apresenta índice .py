...
# correlação entre as colunas 'IRRF' e 'Diferenca_Salario' por órgão
orgao_correlacao = df_servidores.groupby('ÓRGÃO')[['IRRF', 'Diferenca_Salario']].corr().iloc[0::2, -1]

# órgão com o maior índice de correlação
orgao_maior_corr = orgao_correlacao.idxmax()

#valor da maior correlação
maior_correlacao = orgao_correlacao.max()

print(f"O órgão que apresenta índice de correlação entre IRRF e Diferenca_Salario maior é '{orgao_maior_corr}' com um índice de correlação de {maior_correlacao:.2f}.")